...
class SuperclassAccuracyMetric(object):
    def __init__(self, topk: typing.Iterable[int] = (1,), n_superclass: int = 10):
        self.topk = sorted(list(topk))
        self.n_superclass = n_superclass
        self.reset()

    def reset(self) -> None:
        self.accuracies = [[Accuracy() for superclass_idx in range(self.n_superclass)] for _ in self.topk]

    def update(self, outputs, targets, super_targets) -> None:
        maxk = max(self.topk)
        batch_size = targets.size(0)
        ...
        for accuracy, k in zip(self.accuracies, self.topk):
            correct_k = correct[:k]
            for superclass_accuracy, superclass_idx in zip(accuracy, list(range(self.n_superclass))):
                non_zero_idx = (super_targets == superclass_idx).nonzero().reshape(-1)
                selected_correct_k = torch.index_select(correct_k, dim=1, index=non_zero_idx).sum().item()
                superclass_accuracy.update(selected_correct_k, non_zero_idx.nelement())

    def at(self, topk: int) -> Accuracy:
        if topk not in self.topk:
            raise ValueError(f"topk={topk} is not in registered topks={self.topk}")
        accuracy = self.accuracies[self.topk.index(topk)]
        for superclass_idx in range(self.n_superclass):
            accuracy[superclass_idx].sync()
        return accuracy
...
class ConfusionMatrix(object):
    ...
    def accuracy(self):
        self.sync()
        m = self._matrix_since_last_sync.float()
        diag = m.diag()
        return (diag / m.sum(1)).tolist()
