...
from codebase.third_party.spos_ofa.ofa.utils import download_url
from codebase.third_party.spos_ofa.ofa.imagenet_classification.networks import get_net_by_name, proxyless_base
from codebase.third_party.spos_ofa.ofa.imagenet_classification.elastic_nn.networks import OFAMobileNetV3, OFAProxylessNASNets, OFAResNets
...
def ofa_net(net_id, pretrained=True):
	...
	if pretrained:
		...
		init = torch.load(
			download_url(
				'https://hanlab.mit.edu/files/OnceForAll/ofa_nets/' + net_id,
                model_dir='/apdcephfs/share_1318108/huberyniu/ofa_exps/.torch/ofa_checkpoints'),
			map_location='cpu')['state_dict']
		net.load_state_dict(init)
	return net
