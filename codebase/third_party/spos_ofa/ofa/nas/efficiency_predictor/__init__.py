...
from codebase.torchutils.common import torchsave, auto_device, unwarp_module


class BaseEfficiencyModel:

    def __init__(self, ofa_net, num_classes_per_superclass=1000):
        self.ofa_net = unwarp_module(ofa_net)
        self.num_classes_per_superclass = num_classes_per_superclass

    def get_active_subnet_config(self, arch_dict):
        arch_dict = copy.deepcopy(arch_dict)
        image_size = arch_dict.pop('image_size')
        superclass_id = arch_dict.pop('superclass_id')
        self.ofa_net.set_active_subnet(**arch_dict)
        active_net_config = self.ofa_net.get_active_net_config()
        return active_net_config, image_size, superclass_id
...
class Mbv3FLOPsModel(BaseEfficiencyModel):
    ...
    def get_efficiency(self, arch_dict):
        active_net_config, image_size, superclass_id = self.get_active_subnet_config(arch_dict)
        return MBv3LatencyTable.count_flops_given_config(active_net_config, image_size, self.num_classes_per_superclass)
...
class PreResNetFLOPsModel(BaseEfficiencyModel):

    def get_efficiency(self, arch_dict):
        active_net_config, image_size, superclass_id = self.get_active_subnet_config(arch_dict)
        return PreResNetLatencyTable.count_flops_given_config(active_net_config,
                                                              image_size,
                                                              superclass_id,
                                                              self.num_classes_per_superclass)
