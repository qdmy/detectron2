...
import yaml
from codebase.third_party.spos_ofa.ofa.utils import download_url, make_divisible, MyNetwork

__all__ = ['count_conv_flop', 'ProxylessNASLatencyTable', 'MBv3LatencyTable', 'ResNet50LatencyTable', 'PreResNetLatencyTable']
...
class MBv3LatencyTable(LatencyTable):
	...
	@staticmethod
	def count_flops_given_config(net_config, image_size=224, num_classes_per_superclass=1000):
		flops = 0
		# first conv
		fsize = int((image_size - 1) / net_config['first_conv']['stride'] + 1)
		layer_flops = count_conv_flop(fsize, 3, net_config['first_conv']['out_channels'], 3, 1)
		flops += layer_flops
		...
		for block in net_config['blocks']:
			...
			if mb_conv['expand_ratio'] != 1:
				# inverted bottleneck
				block_flops += count_conv_flop(fsize, mb_conv['in_channels'], mb_conv['mid_channels'], 1, 1)
				# print(f"in_channels: {mb_conv['in_channels']}, mid_channels: {mb_conv['mid_channels']}, img size: {fsize}, stride: {mb_conv['stride']}")
				# print(f"inverted bottleneck: {count_conv_flop(fsize, mb_conv['in_channels'], mb_conv['mid_channels'], 1, 1)}")
			# depth conv
			block_flops += count_conv_flop(out_fz, mb_conv['mid_channels'], mb_conv['mid_channels'],
			                         mb_conv['kernel_size'], mb_conv['mid_channels'])
			# print(f"depth conv: {count_conv_flop(out_fz, mb_conv['mid_channels'], mb_conv['mid_channels'],mb_conv['kernel_size'], mb_conv['mid_channels'])}")
			if mb_conv['use_se']:
				# SE layer
				se_mid = make_divisible(mb_conv['mid_channels'] // 4, divisor=MyNetwork.CHANNEL_DIVISIBLE)
				block_flops += count_conv_flop(1, mb_conv['mid_channels'], se_mid, 1, 1)
				block_flops += count_conv_flop(1, se_mid, mb_conv['mid_channels'], 1, 1)
				# print(f"se: {count_conv_flop(1, mb_conv['mid_channels'], se_mid, 1, 1) + count_conv_flop(1, se_mid, mb_conv['mid_channels'], 1, 1)}")
			# point linear
			block_flops += count_conv_flop(out_fz, mb_conv['mid_channels'], mb_conv['out_channels'], 1, 1)
			# print(f"point linear: {count_conv_flop(out_fz, mb_conv['mid_channels'], mb_conv['out_channels'], 1, 1)}")
			flops += block_flops
			fsize = out_fz
...
class PreResNetLatencyTable(LatencyTable):
	...
	@staticmethod
	def count_flops_given_config(net_config, image_size=224, superclass_id=0, num_classes_per_superclass=1000):
		flops = 0
		# input stem
		for layer_config in net_config['input_stem']:
			if layer_config['name'] != 'ConvLayer':
			...
			layer_flops = count_conv_flop(out_image_size, in_channel, out_channel,
			                         layer_config['kernel_size'], layer_config.get('groups', 1))
			flops += layer_flops
			image_size = out_image_size
		...
		for block_config in net_config['blocks']:
			...
			in_channel = block_config['in_channels']
			out_channel = block_config['out_channels']
			max_out_channel = block_config['max_out_channels']

			out_image_size = int((image_size - 1) / block_config['stride'] + 1)

			# conv1
			conv1_flops = count_conv_flop(out_image_size, in_channel, out_channel,
			                         block_config['kernel_size'], block_config['groups'])
			flops += conv1_flops
			# conv2
			conv2_flops = count_conv_flop(out_image_size, out_channel, max_out_channel,
                                     block_config['kernel_size'], block_config['groups'])
			flops += conv2_flops
            
			# downsample
			downsample_flops = 0
			if block_config['stride'] == 1 and in_channel == max_out_channel:
				pass
			else:
				downsample_flops = count_conv_flop(out_image_size, in_channel, max_out_channel, 1, 1)
				flops += downsample_flops
			# print(f"Block idx {idx}: {conv1_flops+conv2_flops+downsample_flops}")
			image_size = out_image_size
			idx += 1
		# final classifier
		layer_flops = count_conv_flop(1, net_config['classifier']['in_features'],
