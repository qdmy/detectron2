...
import random
import numpy as np
from codebase.third_party.spos_ofa.ofa.imagenet_classification.networks import ResNets
from codebase.third_party.spos_ofa.ofa.imagenet_classification.networks import PreResNets
...
class MobileNetArchEncoder:
    ...
    def __init__(self,
        ...
        self.superclass_list = list(range(20)) if superclass_list is None else superclass_list
        ...
        # superclass
        self.sc_info = dict(id2val={}, val2id={}, L=[], R=[])
        self._build_info_dict(target='sc')
    ...
    def _build_info_dict(self, target):
        if target == 'r':
            ...
            target_dict = self.sc_info
            target_dict['L'].append(self.n_dim)
            for supeclass_id in self.superclass_list:
                target_dict['val2id'][supeclass_id] = self.n_dim
                target_dict['id2val'][self.n_dim] = supeclass_id
                self.n_dim += 1
            target_dict['R'].append(self.n_dim)
        else:
            if target == 'k':
    ...
    def arch2feature(self, arch_dict):
        ...
        feature[self.sc_info['val2id'][sc]] = 1
        return feature
    ...
    def feature2arch(self, feature):
        ...
        superclass_id = self.sc_info['id2val'][
            int(np.argmax(feature[self.sc_info['L'][0]:self.sc_info['R'][0]])) + self.sc_info['L'][0]
            ]
        assert superclass_id in self.superclass_list
        arch_dict = {'ks': [], 'e': [], 'd': [], 'image_size': img_sz, 'supercclass_id': superclass_id}
    ...
    def random_sample_arch(self, superclass_id):
        return {
...
class MobileNetAllTaskArchEncoder:
    SPACE_TYPE = 'mbv3'
...
class PreResNetArchEncoder:
    ...
    def __init__(self,
        ...
        self.ks_list = [3, 5, 7] if ks_list is None else ks_list
        self.depth_list = [7, 8, 9] if depth_list is None else depth_list
        self.width_mult_list = [0.50, 0.80, 1.0] if width_mult_list is None else width_mult_list

        self.base_depth_list = PreResNets.BASE_DEPTH_LIST if base_depth_list is None else base_depth_list
        self.superclass_list = list(range(20)) if superclass_list is None else superclass_list
    ...
    def arch2feature(self, arch_dict):
        d, w, ks = arch_dict['d'], arch_dict['w'], arch_dict['ks']
        r, sc = arch_dict['image_size'], arch_dict['superclass_id']
    ...
    def feature2arch(self, feature):
        ...
        assert superclass_id in self.superclass_list
        arch_dict = {'d': [], 'w': [], 'ks': [], 'image_size': img_sz, 'superclass_id': superclass_id}
        ...
        for i in range(self.max_n_blocks):
            ...
            for j in range(self.width_mult_info['L'][i], self.width_mult_info['R'][i]):
                if feature[j] == 1:
                    arch_dict['w'].append(self.width_mult_info['id2val'][i][j])
                    assert not skip
                    skip = False
                    break

            if skip:
                arch_dict['w'].append(0)
                arch_dict['ks'].append(0)
            else:
                d += 1
    ...
    def random_sample_arch(self, superclass_id):
        return {
            'ks': random.choices(self.ks_list, k=self.max_n_blocks),
            'd': random.choices(self.depth_list, k=self.n_stage),
            'w': random.choices(self.width_mult_list, k=self.max_n_blocks),
            'image_size': random.choice(self.image_size_list),
            'superclass_id': int(superclass_id)
        }
    ...
    def mutate_arch(self, arch_dict, mutate_prob):
        ...
        for i in range(self.max_n_blocks):
            if random.random() < mutate_prob:
                arch_dict['ks'][i] = random.choice(self.ks_list)
                arch_dict['w'][i] = random.choice(self.width_mult_list)
