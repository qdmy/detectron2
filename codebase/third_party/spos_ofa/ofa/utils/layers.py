...
from ..utils import get_same_padding, min_divisible_value, SEModule, ShuffleLayer
from ..utils import MyNetwork, MyModule
from ..utils import build_activation, make_divisible
from detectron2.layers.batch_norm import get_norm

__all__ = [
    'set_layer_from_config',
    'ConvLayer', 'IdentityLayer', 'LinearLayer', 'MultiHeadLinearLayer', 'ZeroLayer', 'MBConvLayer',
    'ResidualBlock', 'ResNetBottleneckBlock', 'PreResNetBasicBlock'
]
...
def set_layer_from_config(layer_config):
    ...
    name2layer = {
        ...
        PreResNetBasicBlock.__name__: PreResNetBasicBlock,
    }
...
class My2DLayer(MyModule):
    ...
    def __init__(self, in_channels, out_channels,
                 use_bn=True, act_func='relu', dropout_rate=0, ops_order='weight_bn_act', norm="BN"):
        ...
        if self.use_bn:
            if self.bn_before_weight:
                modules['bn'] = get_norm(norm, in_channels) # nn.BatchNorm2d(in_channels)
            else:
                modules['bn'] = get_norm(norm, out_channels) # nn.BatchNorm2d(out_channels)
        else:
            modules['bn'] = None
        # activation
...
class ConvLayer(My2DLayer):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        super(ConvLayer, self).__init__(in_channels, out_channels, use_bn, act_func, dropout_rate, ops_order, norm=norm)
        if self.use_se:
    ...
    def module_str(self):
        ...
        if self.use_bn:
            if isinstance(self.bn, nn.GroupNorm):
                ...
                conv_str += '_BN'
            elif isinstance(self.bn, nn.SyncBatchNorm):
                conv_str += '_SyncBN'
        return conv_str
...
class MBConvLayer(MyModule):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        if self.expand_ratio == 1:
            ...
            self.inverted_bottleneck = nn.Sequential(OrderedDict([
                ...
                ('bn', get_norm(norm, feature_dim)), # nn.BatchNorm2d(feature_dim)),
        ...
        self.point_linear = nn.Sequential(OrderedDict([
            ...
            ('bn', get_norm(norm, out_channels)), # nn.BatchNorm2d(out_channels)),
        ]))
    ...
    def module_str(self):
        ...
        if isinstance(self.point_linear.bn, nn.GroupNorm):
            ...
            layer_str += '_BN'
        elif isinstance(self.point_linear.bn, nn.SyncBatchNorm):
            layer_str += '_SyncBN'
...
class PreResNetBasicBlock(MyModule):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        self.conv2 = nn.Sequential(OrderedDict([
            ('bn', nn.BatchNorm2d(out_channels)),
            ('act', build_activation(self.act_func, inplace=True)),
            ('conv', nn.Conv2d(out_channels, out_channels, kernel_size, 1, pad, bias=False)),
        ]))
    ...
    def module_str(self):
        return '(%s, %s)' % (
            '%d%d_PreResNetBasicBlockConv_%d->%d_S%d' % (
                self.kernel_size, self.kernel_size, self.in_channels, self.out_channels,
                self.stride
            ),
            'Identity' if isinstance(self.downsample, IdentityLayer) else self.downsample_mode,
    ...
    @staticmethod
    def build_from_config(config):
        return PreResNetBasicBlock(**config)
