...
def cross_entropy_loss_with_soft_target(pred, soft_target):
	assert pred.shape == soft_target.shape, "pred and target should have the same dimension"
	n_class = pred.shape[-1]
	pred = pred.view(-1, n_class)
	soft_target = soft_target.view(-1, n_class)
	# logsoftmax = nn.LogSoftmax(dim=1) # 先做softmax，再做log，这个函数已经在得到pred的时候做好了
	# loss = torch.mean(torch.sum(- soft_target * logsoftmax(pred), 1))
	loss = torch.sum(- soft_target * pred)
	return loss
