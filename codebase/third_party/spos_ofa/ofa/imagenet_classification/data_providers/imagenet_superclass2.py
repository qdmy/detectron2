...
from codebase.third_party.spos_ofa.ofa.imagenet_classification.data_providers.cifar2 import Subset
...
def split_train_val(dataset, val_num, seed):
    ...
    for sample_indices in dataset.superclass_samples_indices:
        ...
        superclass_train_indices.append(np.array(superclass_indices)[permutation[:split]].tolist())
    ...
    return total_train_indices, total_val_indices, superclass_train_indices, superclass_val_indices
...
def get_imagenet_superclass_train_val_loader(args, train_transform, test_transform, class_ranges):
    ...
    results = split_train_val(dataset,
                              args.val_num,
                              args.seed)
    total_train_indices, total_val_indices, superclass_train_indices, superclass_val_indices = results
...
class CustomDatasetFolder(VisionDataset):
    ...
    def __init__(
        ...
        for i in range(self.n_superclass):
            ...
            self.superclass_data.append(np.array(self.samples)[idx].tolist())
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        if self.train:
            ...
            path, _ = self.superclass_data[self.superclass_id][index]
            target = self.superclass_targets[self.superclass_id][index]
