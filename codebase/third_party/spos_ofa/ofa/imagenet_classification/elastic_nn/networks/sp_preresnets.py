...
from codebase.third_party.spos_ofa.ofa.imagenet_classification.elastic_nn.modules.dynamic_single_path_layers import \
    DynamicPreResNetSinglePathBasicBlock
...
class OFAPreResNets(PreResNets):
    ...
    def __init__(self, n_classes=100, bn_param=(0.1, 1e-5), dropout_rate=0,
        ...
        for d, width, s in zip(n_block_list, stage_width_list, stride_list):
            for i in range(d):
                ...
                bottleneck_block = DynamicPreResNetSinglePathBasicBlock(
    ...
    def forward(self, x, cum_indicators):
        for layer in self.input_stem:
            x = layer(x)
        for stage_id, (block_idx, cum_indicator) in enumerate(zip(self.grouped_block_index, cum_indicators)):
            depth_param = self.runtime_depth[stage_id]
            active_idx = block_idx[:len(block_idx) - depth_param]
            for idx in active_idx:
                x = self.blocks[idx](x, cum_indicator[0])
        x = self.bn(x)
        x = self.relu(x)
    ...
    def get_flops(self, cum_indicators, image_size=32, num_class_per_superclass=5):
        total_flops = 0
        ...
        for block_idx, block in enumerate(self.blocks):
            ...
            if not isinstance(block.downsample, IdentityLayer):
                group_idx += 1
            cum_indicator = cum_indicators[group_idx][0]
            in_channel = block.in_channel_list[-1]
            max_out_channel = block.out_channel_list[-1]
            out_channel = (block.delta_out_channels * cum_indicator).sum()

            # conv1
            block_flops += count_conv_flop(out_image_size, in_channel, out_channel,
                                           block.kernel_size, 1)

            # conv2
            block_flops += count_conv_flop(out_image_size, out_channel, max_out_channel,
                                           block.kernel_size, 1)

            if isinstance(block.downsample, nn.Sequential):
                block_flops += count_conv_flop(out_image_size, in_channel, max_out_channel, 1, 1)
