...
from codebase.third_party.spos_ofa.ofa.imagenet_classification.elastic_nn.modules.dynamic_single_path_layers import DynamicSinglePathMBConvLayer, SinglePathResidualBlock
from codebase.third_party.spos_ofa.ofa.imagenet_classification.networks import MobileNetV3Cifar, MobileNetV3
...
__all__ = ['OFAMobileNetV3', 'build_sp_ofa_mbv3_backbone']


# class OFAMobileNetV3(MobileNetV3Cifar): # 改为继承MobileNetV3，都是一样的，除了first conv的stride不同
class OFAMobileNetV3(MobileNetV3):
    ...
    def __init__(self, n_classes=1000, bn_param=(0.1, 1e-5), dropout_rate=0.1, base_stage_width=None, width_mult=1.0,
        ...
        for width, n_block, s, act_func, use_se in zip(width_list[2:], n_block_list[1:],
            ...
            for i in range(n_block):
                if i == 0:
                    ...
                    stride = 1
                mobile_inverted_conv = DynamicSinglePathMBConvLayer(
                ...
                if stride == 1 and feature_dim == output_channel:
                    ...
                    shortcut = None
                blocks.append(SinglePathResidualBlock(mobile_inverted_conv, shortcut))
        ...
        for depth in self.depth_list:
            depth_mask = torch.ones(max_depth)
            depth_mask *= nn.functional.pad(torch.ones(depth), [0, max_depth - depth], value=0)
            if prev_depth:
                depth_mask *= nn.functional.pad(torch.zeros(prev_depth),
                                                [0, max_depth - prev_depth], value=1)
            prev_depth = depth
            depth_masks.append(depth_mask)
        self.register_buffer("depth_masks", torch.stack(depth_masks, dim=0))
    ...
    def forward(self, x, depth_cum_indicators, ratio_cum_indicators, kernel_size_cum_indicators): # depths=None, ratios=None, kernel_sizes=None
        res = {}
        # first conv
        x = self.first_conv(x)
        # first block
        ...
        # blocks
        for stage_id, (
                block_idx, stage_depth_cum_indicators, stage_ratio_cum_indicators,
                stage_kernel_size_cum_indicators) in enumerate(
            zip(self.block_group_info, depth_cum_indicators, ratio_cum_indicators, kernel_size_cum_indicators)):
            # depth = self.runtime_depth[stage_id]
            active_idx = block_idx
            stage_depth_cum_indicators = stage_depth_cum_indicators[0].unsqueeze(-1)
            depth_masks = (self.depth_masks * stage_depth_cum_indicators).sum(0)
            for idx, depth_mask, ratio_cum_indicator, kernel_size_cum_indicator in zip(active_idx,
                                                                                       depth_masks,
                                                                                       stage_ratio_cum_indicators,
                                                                                       stage_kernel_size_cum_indicators):
                if self.blocks[idx].conv.stride == 1 and \
                        max(self.blocks[idx].conv.in_channel_list) == max(self.blocks[idx].conv.out_channel_list):
                    x = self.blocks[idx](x, ratio_cum_indicator, kernel_size_cum_indicator) * depth_mask + \
                        x * (1.0 - depth_mask)
                else:
                    x = self.blocks[idx](x, ratio_cum_indicator, kernel_size_cum_indicator) * depth_mask

            stage_name = 'res' + str(stage_id+1)
            res[stage_name] = x
    ...
    def get_active_subnet(self, preserve_weight=True):
        ...
        _subnet = MobileNetV3Cifar(first_conv, blocks, tuple(stage_names), out_feature_channels, out_feature_strides, block_index) #, final_expand_layer, feature_mix_layer, classifier)
    ...
    def get_flops(
            ...
            kernel_size_cum_indicators,
            image_size=224,
            num_class_per_superclass=5
    ):
        total_flops = 0

        # first conv
        op = self.first_conv[0] # 有[0]是因为我手贱的加上了nn.sequential
        in_channel = 3
        ...
        for stage_id, (
            ...
            for idx, depth_mask, ratio_cum_indicator, kernel_size_cum_indicator in zip(active_idx,
                ...
                # inverted_bottleneck
                in_channels = block.in_channels
                mid_channels = (block.delta_middle_channels * ratio_cum_indicator[0]).sum()
                out_channels = block.out_channels
                kernel_size = (block.depth_conv.conv.delta_kernel_sizes * kernel_size_cum_indicator[0]).sum()
                stride = block.stride
                out_fz = int((fsize - 1) / stride + 1)
                ...
                # point linear
                block_flops += count_conv_flop(out_fz, mid_channels, out_channels, 1, 1)
                total_flops += block_flops * depth_mask
                # if depth_mask == 1:
                    # if max(block.expand_ratio_list) != 1:
                        # print(f'in_channels: {in_channels}, mid_channels: {mid_channels}, img size: {fsize}, stride: {stride}')
                        # print(f"inverted bottleneck: {count_conv_flop(fsize, in_channels, mid_channels, 1, 1)}")
                ...
                fsize = out_fz
                    ...
                    # print(
                    #     f'in_channels: {in_channels}, mid_channels: {mid_channels}, out_channels: {out_channels}, kernel_size: {kernel_size}, stride: {stride}, img size: {out_image_size}')
...
def build_sp_ofa_mbv3_backbone(cfg, input_shape):
    """
