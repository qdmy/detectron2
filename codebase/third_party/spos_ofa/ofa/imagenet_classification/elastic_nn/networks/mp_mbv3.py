...
__all__ = ['OFAMobileNetV3', 'build_mp_ofa_mbv3_backbone']
...
class OFAMobileNetV3(MobileNetV3):
    ...
    def forward(self, x, depths_probs, ratios_probs, ks_probs): # depths=None, ratios=None, kernel_sizes=None
        res = {}
        # first conv
        x = self.first_conv(x)
        # first block
                ...
                for idx, block_ratios_probs, block_ks_probs in zip(active_idx, stage_ratios_probs, stage_ks_probs):
                    ...
                    for ratio_idx, ratio in enumerate(self.expand_ratio_list):
                        ...
                        for ks_idx, ks in enumerate(self.ks_list):
                            ...
                            ratios_ks_avg_output += self.blocks[idx](block_input) * block_ks_probs[0][ks_idx] * block_ratios_probs[0][ratio_idx]
                            # TODO: 这个forward的时候显存增的很多，导致oom

                    stage_output = ratios_ks_avg_output
                    block_input = stage_output
                depth_avg_output += block_input * stage_depths_probs[0][depth_idx]
    ...
    def get_flops(self, depths_probs, ratios_probs, ks_probs, image_size=224, num_class_per_superclass=5):
        total_flops = 0
...
def build_mp_ofa_mbv3_backbone(cfg, input_shape):
    """
    ...
    model = OFAMobileNetV3(
        n_classes=cfg.MODEL.MP_OFA_MOBILENETV3.NUM_CLASSES,
        bn_param=(cfg.MODEL.MP_OFA_MOBILENETV3.BN_MOMENTUM, cfg.MODEL.MP_OFA_MOBILENETV3.BN_EPS),
		width_mult=cfg.MODEL.MP_OFA_MOBILENETV3.WIDTH_MULT_LIST,
        ks_list=cfg.MODEL.MP_OFA_MOBILENETV3.KS_LIST, 
		expand_ratio_list=cfg.MODEL.MP_OFA_MOBILENETV3.EXPAND_LIST, 
		depth_list=cfg.MODEL.MP_OFA_MOBILENETV3.DEPTH_LIST,
		CFG=cfg
    )
    return model
