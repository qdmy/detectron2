...
from codebase.third_party.spos_ofa.ofa.utils import get_same_padding, sub_filter_start_end, make_divisible, SEModule, MyNetwork, MyConv2d
...
class DynamicKernelConv2d(nn.Module):
	...
	def __init__(self, max_in_channels, max_out_channels, kernel_size_list=1, stride=1, dilation=1):
		super(DynamicKernelConv2d, self).__init__()
		...
		self.conv = nn.Conv2d(
			self.max_in_channels, self.max_out_channels, max(self.kernel_size_list), stride=self.stride, bias=False,
		)
	...
	def get_active_filter(self, out_channel, in_channel, kernel_size):
		max_kernel_size = max(self.kernel_size_list)
		start, end = sub_filter_start_end(max_kernel_size, kernel_size)
		return self.conv.weight[:out_channel, :in_channel, start:end, start:end]
...
class DynamicSyncBatchNorm2d(nn.Module):
	...
	def __init__(self, max_feature_dim):
		...
		self.bn = nn.SyncBatchNorm(self.max_feature_dim)

	@staticmethod
	def bn_forward(x, bn: nn.SyncBatchNorm, feature_dim):
		if bn.num_features == feature_dim or DynamicSyncBatchNorm2d.SET_RUNNING_STATISTICS:
			return bn(x)
		else:
			...
			need_sync = (bn_training and bn.training)
			if need_sync:
...
class DynamicSBatchNorm2d(nn.Module):
	SET_RUNNING_STATISTICS = False

	def __init__(self, feature_dim_list, max_feature_dim):
		super(DynamicSBatchNorm2d, self).__init__()

		self.feature_dim_list = feature_dim_list
		self.max_feature_dim = max_feature_dim
		self.bn = nn.ModuleList([nn.BatchNorm2d(feature_dim) for feature_dim in feature_dim_list])
		# self.bn = nn.BatchNorm2d(self.max_feature_dim)
	...
	def forward(self, x):
		feature_dim = x.size(1)
		selected_idx = self.feature_dim_list.index(feature_dim)
		y = self.bn[selected_idx]
		# y = self.bn_forward(x, self.bn, feature_dim)
		return y
...
def get_dynamic_norm(norm, out_channels):
    """
    Args:
    ...
    if isinstance(norm, str):
        ...
        norms = {
            ...
            "SyncBN": DynamicSyncBatchNorm2d,
            "GN": lambda channels: DynamicGroupNorm(32, channels),
        }
        if norm in norms:
            norm = norms[norm]
        else:
