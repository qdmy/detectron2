...
from codebase.third_party.spos_ofa.ofa.utils.layers import MBConvLayer, ConvLayer, IdentityLayer, set_layer_from_config
from codebase.third_party.spos_ofa.ofa.utils.layers import ResNetBasicBlock, ResNetBottleneckBlock, PreResNetBasicBlock, LinearLayer
from codebase.third_party.spos_ofa.ofa.utils import MyModule, val2list, get_net_device, build_activation, make_divisible, SEModule, MyNetwork
from .dynamic_op import DynamicSeparableConv2d, DynamicConv2d, DynamicBatchNorm2d, DynamicSE, DynamicGroupNorm, DynamicKernelConv2d, DynamicLinear, get_dynamic_norm

__all__ = [
    'adjust_bn_according_to_idx', 'copy_bn',
    'DynamicMBConvLayer', 'DynamicConvLayer', 'DynamicLinearLayer', 'DynamicResNetBasicBlock', 'DynamicResNetBottleneckBlock', 'DynamicPreResNetBasicBlock'
]
...
class DynamicMBConvLayer(MyModule):
    ...
    def __init__(self, in_channel_list, out_channel_list,
        ...
        self.mid_channel_list = [make_divisible(
            round(max(self.in_channel_list) * ratio), MyNetwork.CHANNEL_DIVISIBLE) for ratio in self.expand_ratio_list]
...
class DynamicPreResNetBasicBlock(MyModule):

    def __init__(self, in_channel_list, out_channel_list, 
                 kernel_size_list, stride=1, act_func='relu',
                 downsample_mode='conv', block_type="both_preact"):
        super(DynamicPreResNetBasicBlock, self).__init__()
        ...
        self.conv1 = nn.Sequential(OrderedDict([
            ...
            ('act', build_activation(self.act_func, inplace=True)),
            ('conv', DynamicKernelConv2d(max(self.in_channel_list), self.max_output_channel, self.kernel_size_list, stride)),
        ]))

        self.conv2 = nn.Sequential(OrderedDict([
            ('bn', DynamicBatchNorm2d(self.max_output_channel)),
            ('act', build_activation(self.act_func, inplace=True)),
            ('conv', DynamicKernelConv2d(self.max_output_channel, self.max_output_channel, self.kernel_size_list)),
        ]))
    ...
    def forward(self, x):
        self.conv1.conv.active_kernel_size = self.active_kernel_size
    ...
    def module_str(self):
        return '(%s, %s)' % (
            '%dx%d_PreResNetBasicBlockConv_in->%d_S%d' % (
                self.active_kernel_size, self.active_kernel_size, self.active_out_channel, self.stride
            ),
            'Identity' if isinstance(self.downsample, IdentityLayer) else self.downsample_mode,
        )
    ...
    def config(self):
        return {
            'name': DynamicPreResNetBasicBlock.__name__,
            'in_channel_list': self.in_channel_list,
            'out_channel_list': self.out_channel_list,
            'kernel_size_list': self.kernel_size_list,
            'kernel_size': self.active_kernel_size,
    ...
    @staticmethod
    def build_from_config(config):
        return DynamicPreResNetBasicBlock(**config)
    ...
    def get_active_subnet(self, in_channel, preserve_weight=True):
        ...
        # copy weight from current layer
        sub_layer.conv1.conv.weight.data.copy_(
            self.conv1.conv.get_active_filter(self.active_out_channel, in_channel, self.active_kernel_size).data)
        copy_bn(sub_layer.conv1.bn, self.conv1.bn.bn)

        sub_layer.conv2.conv.weight.data.copy_(
            self.conv2.conv.get_active_filter(self.max_output_channel,
                                              self.active_out_channel,
                                              self.active_kernel_size).data)
        copy_bn(sub_layer.conv2.bn, self.conv2.bn.bn)

        if not isinstance(self.downsample, IdentityLayer):
            sub_layer.downsample.conv.weight.data.copy_(
                self.downsample.conv.get_active_filter(self.max_output_channel, in_channel).data)
    ...
    def re_organize_middle_weights(self, width_ratio_stage=0):
        ...
        if isinstance(self.conv1.bn, DynamicGroupNorm):
            ...
            for chunk in importance_chunks:
                chunk.data.fill_(torch.mean(chunk))
            importance = torch.cat(importance_chunks, dim=0)
        if width_ratio_stage > 0:
            sorted_out_channel_list = copy.deepcopy(self.out_channel_list)
            sorted_out_channel_list.sort(reverse=True)
            # sorted_expand_list = copy.deepcopy(self.expand_ratio_list)
            # sorted_expand_list.sort(reverse=True)
            # target_width_list = [
            #     make_divisible(round(max(self.out_channel_list) * expand), MyNetwork.CHANNEL_DIVISIBLE)
            ...
            # ]
            right = len(importance)
            base = - len(sorted_out_channel_list) * 1e5
            for i in range(width_ratio_stage + 1):
                left = sorted_out_channel_list[i]
                importance[left:right] += base
                base += 1e5
                right = left
        sorted_importance, sorted_idx = torch.sort(importance, dim=0, descending=True)
